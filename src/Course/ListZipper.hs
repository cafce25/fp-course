...
instance Functor MaybeListZipper where
  ...
  f <$> MLZ o = optional (isZ . (f <$>)) isNotZ o
...
--toList (ListZipper (l :. ls) c r) = toList $ ListZipper ls l (c :. r)
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft pred (ListZipper l x r) =
  let (ln, li) = break pred l
   in case li of
        Nil -> isNotZ
        (m :. li') -> isZ $ ListZipper li' m (toList $ ListZipper ln x r)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight pred (ListZipper l x r) =
  let (rn, ri) = break pred r
   in case ri of
        Nil -> isNotZ
        (m :. ri') -> isZ $ ListZipper (reverse . toList $ ListZipper l x rn) m ri'
...
moveLeftN n z
  | n == 0 = isZ z
  | n < 0 = moveRightN (-n) z
  | otherwise = optional (moveLeftN (n-1)) isNotZ (toOptional $ moveLeft z)
...
moveRightN n z
  | n == 0 = isZ z
  | n < 0 = moveLeftN (-n) z
  | otherwise = optional (moveRightN (n-1)) isNotZ (toOptional $ moveRight z)
...
moveLeftN' n z@(ListZipper l _ _) =
  if n < 0
     then moveRightN' (-n) z
     else case toOptional $ moveLeftN n z of
            Empty -> Left $ length l
            Full z' -> Right z'
...
moveRightN' n z@(ListZipper _ _ r) =
  if n < 0
     then moveLeftN' (-n) z
     else case toOptional $ moveRightN n z of
            Empty -> Left $ length r
            Full z' -> Right z'
...
-- /Tip:/ Use @List#repeat@.
  pure el = ListZipper (repeat el) el (repeat el)
-- /Tip:/ Use `zipWith`
  ListZipper fl fc fr <*> ListZipper al ac ar =
    ListZipper (zipWith ($) fl al) (fc ac) (zipWith ($) fr ar)
...
instance Extend ListZipper where
  f <<= z@(ListZipper l _ r) =
    ListZipper  (map f l') (f z) (map f r')
    where l' = take (length l) . drop 1 . listh . P.iterate moveLeftLoop $ z
          r' = take (length r) . drop 1 . listh . P.iterate moveRightLoop $ z
