...
hex = do
  ...
  pure $ optional chr (error "never") (readHex digits)
...
eof ::
  Parser ()
eof = P (\case Nil -> Result Nil ()
               cs -> UnexpectedString cs)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll preds = satisfy (and . sequence preds) -- satisfy (all $ sequence preds)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny preds = satisfy (or . sequence preds) -- satisfy (all $ sequence preds)
